...
app.config['YANDEX_MAPS_API_KEY'] = os.getenv('YANDEX_MAPS_API_KEY', 'YOUR_YANDEX_MAPS_API_KEY')
...
ALLOWED_ORDER_STATUS_KEYS = {key for key, _ in ORDER_STATUS_FLOW}
ALLOWED_ORDER_STATUS_KEYS.add('cancelled')
...
def seller_order_details(order_id):
    ...
    return render_template(
        ...
        status_choices=ORDER_STATUS_FLOW
    )
...
def update_order_status(order_id):
    ...
    if new_status not in ALLOWED_ORDER_STATUS_KEYS:
        flash('Некорректный статус заказа', 'error')
        return redirect(url_for('seller_order_details', order_id=order_id))

    next_url = request.form.get('next') or request.referrer or url_for('seller_order_details', order_id=order_id)
    parsed = urlparse(next_url)
    if parsed.netloc:
        next_url = url_for('seller_order_details', order_id=order_id)
    ...
    owns_items = conn.execute(
        'SELECT 1 FROM order_items WHERE order_id = ? AND seller_id = ?',
        (order_id, user_id)
    ).fetchone()
    if user_role != 'admin' and not owns_items:
        conn.close()
        flash('Недостаточно прав для изменения статуса', 'error')
        return redirect(url_for('account'))
...
def update_account_settings():
    ...
    conn = get_db()
    user = conn.execute('SELECT id, username, email, password FROM users WHERE id = ?', (session['user_id'],)).fetchone()
    if not user:
        conn.close()
    ...
    if any([new_password, confirm_password]):
        ...
        if not current_password:
            conn.close()
            flash('Для смены пароля укажите текущий пароль', 'error')
            return redirect(url_for('account'))
...
def create_admin_user():
    ...
    if not all([username, email, password, confirm_password]):
        flash('Заполните все поля для добавления администратора', 'error')
        return redirect(url_for('admin_users'))
    ...
    if len(password) < 6:
        flash('Пароль администратора должен быть не короче 6 символов', 'error')
        return redirect(url_for('admin_users'))
    ...
    hashed_password = generate_password_hash(password)
    conn.execute(
        '''INSERT INTO users (username, email, password, role, seller_request_status)
           VALUES (?, ?, ?, 'admin', 'none')''',
...
def promote_user_to_admin():
    ...
    try:
        ...
        flash('Некорректный пользователь для повышения', 'error')
        return redirect(url_for('admin_users'))

    if user_id == session.get('user_id'):
        flash('Вы уже являетесь администратором', 'info')
        return redirect(url_for('admin_users'))
    ...
    conn.execute("UPDATE users SET role = 'admin', seller_request_status = 'none' WHERE id = ?", (user_id,))
    conn.commit()
    conn.close()
...
def confirm_order_item(order_id, order_item_id):
    ...
    order_item = conn.execute('''
        SELECT oi.*, o.user_id as buyer_id, oi.seller_id, p.created_by_user_id as product_owner_id
        FROM order_items oi
    ...
    user_role = session.get('role')
    is_admin = user_role == 'admin'
    owns_by_seller = order_item['seller_id'] == session['user_id']
    owns_by_product = order_item['product_owner_id'] == session['user_id'] if order_item['product_owner_id'] else False
    if not (is_admin or owns_by_seller or owns_by_product):
        conn.close()
