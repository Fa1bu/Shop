...
def checkout():
    conn = get_db()
    cart_items = conn.execute('''SELECT c.*, p.name, p.price, p.image, p.id as product_id
                                 FROM cart c
                                 JOIN products p ON c.product_id = p.id
                                 WHERE c.user_id = ?''',
                             (session['user_id'],)).fetchall()
    ...
    if request.method == 'POST':
        ...
        # Создаем заказ
        cursor = conn.execute(
            '''INSERT INTO orders (user_id, total_amount, customer_name, customer_phone, delivery_address, comment, status)
               VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (session['user_id'], total, customer_name, customer_phone, delivery_address, comment, 'pending')
        )
