    ...
    if request.method == 'POST':
        ...
        cursor = conn.execute(
            '''INSERT INTO orders (user_id, total_amount, customer_name, customer_phone, delivery_address, comment, payment_status, payment_reference, status)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)''',
            (session['user_id'], total, customer_name, customer_phone, delivery_address, comment, 'pending', payment_reference or None, 'pending')
        )
        ...
        flash('Заказ успешно оформлен! Номер заказа: #' + str(order_id), 'success')
...
def account():
    ...
    if user['role'] in ('seller', 'admin'):
        seller_products = conn.execute(
            'SELECT id, name, price, is_approved, created_by_user_id FROM products WHERE created_by_user_id = ? ORDER BY id DESC',
            (session['user_id'],)
        ).fetchall()
    
    # Получаем товары других пользователей для одобрения (только для продавцов)
    products_to_approve = []
    if user['role'] in ('seller', 'admin'):
        if user['role'] == 'admin':
            # Админы видят все неодобренные товары
            products_to_approve = conn.execute(
                'SELECT id, name, price, is_approved, created_by_user_id FROM products WHERE is_approved = 0 OR is_approved IS NULL ORDER BY id DESC'
            ).fetchall()
        else:
            # Продавцы видят только товары других пользователей
            products_to_approve = conn.execute(
                'SELECT id, name, price, is_approved, created_by_user_id FROM products WHERE (is_approved = 0 OR is_approved IS NULL) AND created_by_user_id != ? ORDER BY id DESC',
                (session['user_id'],)
            ).fetchall()
...
def approve_product(product_id):
    ...
    # Проверяем, что пользователь является продавцом/админом и НЕ является создателем товара
    ...
    if not is_seller:
        conn.close()
        flash('Только продавцы могут одобрять товары', 'error')
        return redirect(url_for('account'))
    
    if is_creator and not is_admin:
        conn.close()
        flash('Вы можете одобрять только товары других пользователей', 'error')
        return redirect(url_for('account'))
...
def unapprove_product(product_id):
    ...
    if not is_seller:
        conn.close()
        flash('Только продавцы могут отменять одобрение товаров', 'error')
        return redirect(url_for('account'))
    
    if is_creator and not is_admin:
        conn.close()
        flash('Вы можете отменять одобрение только товаров других пользователей', 'error')
        return redirect(url_for('account'))
